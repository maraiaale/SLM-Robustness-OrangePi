...
def wait_server_ready(server_url, timeout_s):
    health_url = server_url.rstrip("/") + "/v1/models"
...
def stream_completion(server_url, prompt, tokens, temperature, timeout_s, model_name="local-model"):
    """
...
def main():
    ...
    if ppl_value:
        print(f"PPL {args.model_name} = {ppl_value}")
    else:
        print(
            "PPL non calcolata per la pipeline NPU/RKLLM; "
            "la colonna CSV viene lasciata vuota."
        )
