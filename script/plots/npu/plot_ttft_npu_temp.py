...
BASE_DIR = Path(r"C:\Users\alema\Desktop\npu_pinning\baseline_qwen_en_npu")
CSV_DIR = BASE_DIR / "results_npu_rkllama_final_30runs"
OUT_DIR = BASE_DIR / "grafici_baseline_npu"
...
def load_temperature_data(temp):
    ...
    for model_file, model_label in MODELS:
        for mode in MODES:
            filename = f"npu_results_{model_file}_temp_{temp}_{mode}.csv"
...
def plot_ttft(df, ttft_col, temp):
    ...
    output_png = OUT_DIR / f"npu_boxplot_ttft_temp_{safe_temp}.png"
    output_pdf = OUT_DIR / f"npu_boxplot_ttft_temp_{safe_temp}.pdf"
    concat_csv = OUT_DIR / f"npu_concat_ttft_temp_{safe_temp}.csv"
    summary_csv = OUT_DIR / f"npu_summary_ttft_temp_{safe_temp}.csv"
