...
BASELINE_DIR = Path(
    r"C:\Users\alema\Desktop\npu_pinning\baseline_qwen_en_npu\results_npu_rkllama_final_30runs"
)

PINNING_DIR = Path(
    r"C:\Users\alema\Desktop\npu_pinning\results_stress_npu_pinning"
)

OUT_DIR = Path(r"C:\Users\alema\Desktop\npu_pinning\grafici_pinning")
...
def find_baseline_csv(model_file: str, temp: str):
    ...
    loose_matches = list(BASELINE_DIR.rglob(f"*{model_file}*temp_{temp}*warm*.csv"))
...
def find_pinning_csv(stressor: str, model_file: str, temp: str):
    ...
    # fallback: cerca in tutta la cartella pinning
    loose_matches = list(PINNING_DIR.rglob(f"*{model_file}*temp_{temp}*warm*.csv"))
    loose_matches += list(PINNING_DIR.rglob(f"*{model_file}*temp_{safe_temp(temp)}*warm*.csv"))
    loose_matches += list(PINNING_DIR.rglob(f"*{model_file}*{temp}*.csv"))
    loose_matches += list(PINNING_DIR.rglob(f"*{model_file}*{safe_temp(temp)}*.csv"))

    loose_matches = sorted(set(loose_matches))
    loose_matches = [p for p in loose_matches if p.is_file()]
    loose_matches = [p for p in loose_matches if stressor.lower() in str(p).lower()]
    loose_matches = [p for p in loose_matches if not is_bad_candidate(p)]
    loose_matches = [p for p in loose_matches if has_any_metric_column(p)]
...
def load_all_data():
    ...
    for temp in TEMPS:
        for model_file, model_label in MODELS:
            ...
            for stressor, stressor_label in STRESSORS:
                ...
                pinning_path = find_pinning_csv(stressor, model_file, temp)
    ...
    missing_path = CSV_DIR / "pinning_missing_files.csv"
...
def build_metric_long_df(all_df: pd.DataFrame):
    ...
    long_path = CSV_DIR / "pinning_all_metrics_long.csv"
...
def make_summary(long_df: pd.DataFrame):
    ...
    summary_path = CSV_DIR / "pinning_summary_stats.csv"
...
def set_scale(ax, scale_type, values):
    ...
    if scale_type == "log":
    ...
    elif scale_type == "symlog_rss":
        ax.set_yscale("symlog", linthresh=160, linscale=1.0)

        ymin = max(values.min() - 10, 120)
        ymax = values.max() + 30
    ...
    elif scale_type == "symlog_cpu":
        ax.set_yscale("symlog", linthresh=1.0, linscale=1.0)

        ymin = -0.05
        ymax = values.max() * 1.25 if values.max() > 0 else 1
        ...
        pad = (ymax - ymin) * 0.12 if ymax > ymin else max(1, ymax * 0.2 if ymax > 0 else 1)
...
def plot_box_metric(metric_df: pd.DataFrame, metric_key: str, cfg: dict, temp: str):
    ...
    out_name = f"npu_pinning_{cfg['output']}_temp_{safe_temp(temp)}"
...
def main():
    print("=== NPU pinning plot generator - singole temperature ===")
    ...
    raw_path = CSV_DIR / "pinning_raw_loaded.csv"
