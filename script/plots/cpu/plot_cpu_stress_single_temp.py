...
STRESS_DIR = Path(
    r"C:\Users\alema\Desktop\cpu\results_stress_final_qwen_cpu_warm_allmodels\results_stress_final"
)

OUT_DIR = Path(
    r"C:\Users\alema\Desktop\cpu\grafici_stress"
)
...
METRICS = {
    "tgs": {
        ...
        "output": "tgs",
        "candidates": [
            "tgs_tokens_s", "tgs", "tgs_s", "TGS", "TGS_s",
            "tokens_per_second", "tok_s", "token_s",
            "generation_speed", "generation_speed_tok_s",
        ],
        "scale": "linear",
        "kind": "box",
    },
    ...
    "total_latency": {
        ...
        "candidates": [
            "total_latency_s", "latency_s", "total_latency",
            "total_time_s", "elapsed_s", "duration_s",
        ],
        "scale": "log",
        "kind": "box",
    },
    "rss": {
        ...
        "output": "rss_peak",
        "candidates": [
            "rss_peak_mb", "rss_mb", "memory_mb", "ram_mb",
            "RSS", "RSS_MB", "resident_set_size_mb",
        ],
        "scale": "linear",
        "kind": "box",
    },
    "cpu_usage": {
        ...
        "output": "cpu_usage",
        "candidates": [
            "cpu_avg_percent", "cpu_percent", "cpu_avg",
    ...
    "board_temp": {
        ...
        "output": "board_temp",
        "candidates": [
            "temp_avg_c", "temperature_avg_c", "board_temp_c",
            "temp_c", "avg_temp_c", "temperature_c",
        ],
        "scale": "linear",
        "kind": "box",
    },
...
def is_bad_candidate(path: Path):
    ...
    bad_words = [
        "summary", "missing", "all_metrics", "raw_loaded",
        "long", "stats", "grafici", "grafico", "concat",
    ]
...
def find_stress_csv(stressor: str, model_file: str, temp: str):
    ...
    direct = STRESS_DIR / model_file / stressor / expected
    if direct.exists():
        return direct

    stressor_root = STRESS_DIR / model_file / stressor
...
def load_all_data():
    ...
    for temp in TEMPS:
        for model_file, model_label in MODELS:
            ...
            for condition, condition_label in CONDITIONS:
                ...
                stress_path = find_stress_csv(condition, model_file, temp)
                ...
                if stress_path is None:
                    missing.append({
                        ...
                        "model": model_file,
                        "temperature": temp,
                        "expected": f"{model_file}\\{condition}\\server_results_{model_file}_temp_{temp}_warm.csv",
                        "error": "not found",
                    })
                    continue
    ...
    if not rows:
        raise FileNotFoundError(
            "Nessun CSV trovato. Controlla BASELINE_DIR e STRESS_DIR."
        )

    all_df = pd.concat(rows, ignore_index=True)

    missing_df = pd.DataFrame(missing)
    missing_path = CSV_DIR / "cpu_stress_missing_files.csv"
...
def build_metric_long_df(all_df: pd.DataFrame):
    ...
    long_path = CSV_DIR / "cpu_stress_all_metrics_long.csv"
...
def make_summary(long_df: pd.DataFrame):
    ...
    summary_path = CSV_DIR / "cpu_stress_summary_stats.csv"
...
def set_scale(ax, scale_type, values):
    ...
    if scale_type == "log":
        ...
        pad = (ymax - ymin) * 0.14 if ymax > ymin else max(
            1,
            ymax * 0.20 if ymax > 0 else 1,
        )
...
def plot_box_metric(metric_df: pd.DataFrame, metric_key: str, cfg: dict, temp: str):
    ...
    offsets = np.linspace(-0.42, 0.42, len(model_labels))
    box_width = 0.22
    ...
    out_name = f"cpu_stress_{cfg['output']}_temp_{safe_temp(temp)}"
...
def main():
    print("=== CPU stress plot generator - singole temperature ===")
    ...
    raw_path = CSV_DIR / "cpu_stress_raw_loaded.csv"
